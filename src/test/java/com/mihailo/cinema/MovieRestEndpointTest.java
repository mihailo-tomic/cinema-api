...
public class MovieRestEndpointTest {
  ...
  public void beforeEveryTest() {
    ...
    this.genres = getMovieGenresList();
    this.actors = getMovieActorsList();

    this.movie = movieRepository.save(getBestMovieEver());

  }

  private List<Genre> getMovieGenresList() {
    List<Genre> genres = new ArrayList<>();

    genres.add(genreRepository.save(new Genre("action")));
    genres.add(genreRepository.save(new Genre("drama")));
    genres.add(genreRepository.save(new Genre("horror")));
    genres.add(genreRepository.save(new Genre("adventure")));
    genres.add(genreRepository.save(new Genre("fantasy")));

    return genres;
  }

  private List<Actor> getMovieActorsList() {
    List<Actor> actors = new ArrayList<>();

    actors.add(actorRepository.save(new Actor("Elijah Wood", "Frodo")));
    actors.add(actorRepository.save(new Actor("Orlando Bloom", "Legolas")));
    actors.add(actorRepository.save(new Actor("Viggo Mortensen", "Aragorn")));
    actors.add(actorRepository.save(new Actor("Ian McKellen", "Gandalf")));

    return actors;
  }

  private Movie getBestMovieEver() {
    return new Movie(
  ...
  private Movie getOtherBestMovieEver() {
    return new Movie(
  ...
  public void testCreateMovie() throws Exception {
    ...
    String movieJson = json(getOtherBestMovieEver());
  ...
  public void testUpdateMovie() throws Exception {
    ...
    this.mockMvc.perform(
        put(ApiConstants.Movie.SINGLE_MOVIE_PATH, movie.getId()).contentType(contentTypeApplicationJson)
            .content(movieJson)
    ).andExpect(
  ...
  public void testSetGenres() throws Exception {
    ...
    this.mockMvc.perform(
        ...
        jsonPath("$..name",
            hasItems(this.genres.get(0).getName(), this.genres.get(1).getName(), this.genres.get(3).getName()))
    ).andDo(
        print()
    );
  ...
  public void testAddGenres() throws Exception {
    ...
    this.mockMvc.perform(
        ...
        jsonPath("$..name",
            hasItems(this.genres.get(0).getName(), this.genres.get(1).getName(), this.genres.get(2).getName(),
                this.genres.get(3).getName(), this.genres.get(4).getName()))
    ).andDo(
        print()
    );
  ...
  public void testSetActors() throws Exception {
    ...
    this.mockMvc.perform(
        ...
        jsonPath("$..name",
            hasItems(this.actors.get(0).getName(), this.actors.get(1).getName(), this.actors.get(3).getName()))
    ).andDo(
        print()
    );
  ...
  public void testAddActors() throws Exception {
    ...
    this.mockMvc.perform(
        ...
        jsonPath("$..name",
            hasItems(this.actors.get(0).getName(), this.actors.get(1).getName(), this.actors.get(2).getName(),
                this.actors.get(3).getName()))
    ).andDo(
        print()
    );
